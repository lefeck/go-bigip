...
func (ifr *IFileResource) List() (*IFileList, error) {
	...
	res, err := ifr.b.RestClient.Get().Prefix(BasePath).ResourceCategory(TMResource).ManagerName(LtmManager).
...
func (ifr *IFileResource) Create(name, fileObject string) error {
	...
	_, err = ifr.b.RestClient.Post().Prefix(BasePath).ResourceCategory(TMResource).ManagerName(LtmManager).
...
func (ifr *IFileResource) Edit(name, fileObject string) error {
	...
	jsonString := string(jsonData)
	_, err = ifr.b.RestClient.Put().Prefix(BasePath).ResourceCategory(TMResource).ManagerName(LtmManager).
...
func (ifr *IFileResource) Delete(name string) error {
	_, err := ifr.b.RestClient.Delete().Prefix(BasePath).ResourceCategory(TMResource).ManagerName(LtmManager).
