...
func (mdr *MonitorDNSResource) List() (*MonitorDNSConfigList, error) {
	var mdcl MonitorDNSConfigList
...
func (mdr *MonitorDNSResource) Get(fullPathName string) (*MonitorDNSConfig, error) {
	var mdc MonitorDNSConfig
	res, err := mdr.b.RestClient.Get().Prefix(ltm.BasePath).ResourceCategory(ltm.TMResource).ManagerName(ltm.LtmManager).
...
func (mdr *MonitorDNSResource) Create(item MonitorDNSConfig) error {
...
func (mdr *MonitorDNSResource) Update(name string, item MonitorDNSConfig) error {
	jsonData, err := json.Marshal(item)
	if err != nil {
		return fmt.Errorf("failed to marshal JSON data: %w", err)
	}
